...
cidr_block = (
    "10.0.0.0/24" if os.environ["DISTRIB_CODENAME"] == "xenial" else "11.0.0.0/24"
)
...
with open(
    "{root}/{env}/clusterman_signals_acceptance.tar.gz".format(
        root=root, env=os.environ["DISTRIB_CODENAME"],
    ),
    "rb",
) as f:
    s3.put_object(
        Bucket="clusterman-signals",
        Key="{env}/clusterman_signals_acceptance.tar.gz".format(
