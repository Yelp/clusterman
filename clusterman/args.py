    ...
    def decorator(add_args):
        def wrapper(subparser):
            subparser = subparser.add_parser(
                command, formatter_class=help_formatter, add_help=False
            )
    ...
    class DeprecationAction(argparse.Action):
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            if option_string == depr_arg:
                logger.warn(
                    f" {depr_arg} is deprecated and will be removed in the future; please use {alt} instead"
                )
...
def get_parser(description=""):  # pragma: no cover
    ...
    root_parser = argparse.ArgumentParser(
        prog="clusterman", description=description, formatter_class=help_formatter
    )
    ...
    root_parser.add_argument(
        "-v", "--version", action="version", version="clusterman " + __version__
    )
