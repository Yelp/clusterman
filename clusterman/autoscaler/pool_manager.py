...
class PoolManager:
    def __init__(
        ...
        find_pending_reason = staticconf.read_bool(f"clusters.{cluster}.find_pending_reason", default=True)
        self.cluster_connector = ClusterConnector.load(self.cluster, self.pool, self.scheduler, find_pending_reason)
    ...
    def _filter_scale_up_options_for_pod(
        ...
        for group_id, options in scale_up_options.items():
            for option in options:
                ...
                if reason:
                    logger.debug("Skipping option {option.instance.market} for {pod.metadata.name}: {reason}")
                    continue
    ...
    def _get_prioritized_killable_nodes(self) -> List[ClusterNodeMetadata]:
        ...
        """
        killable_nodes = [
            metadata for metadata in self.get_node_metadatas(AWS_RUNNING_STATES) if self._is_node_killable(metadata)
        ]
