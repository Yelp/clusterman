...
class Signal:
    def __init__(
        ...
        self.parameters: Dict = {
            key: value
            for param_dict in reader.read_list(
                "autoscale_signal.parameters", default=[]
            )
    ...
    def evaluate(
        ...
        try:
            ...
            metric_bytes = json.dumps(
                {"metrics": metrics, "timestamp": timestamp.timestamp}
            ).encode()
            len_metrics = struct.pack(
                ">I", len(metric_bytes)
            )  # bytes representation of the length, packed big-endian
            self._signal_conn.send(len_metrics)
            response = self._signal_conn.recv(SOCKET_MESG_SIZE)
            if response != ACK:
                raise SignalConnectionError(
                    f"Error occurred sending metric length to signal (response={response})"
                )
            ...
            if ack_bit != ACK:
                raise SignalConnectionError(
                    f"Error occurred sending metric data to signal (response={response})"
                )
            ...
            if retry_on_broken_pipe:
                logger.error(
                    "Signal connection failed; reloading the signal and trying again"
                )
    ...
    @retry(
        exceptions=ConnectionRefusedError, tries=3, delay=5
    )  # retry signal connection in case it's slow to start
    ...
    def _get_metrics(self, end_time: arrow.Arrow) -> MetricsValuesDict:
        ...
        for metric_dict in self.required_metrics:
            ...
            if metric_dict["type"] == SYSTEM_METRICS:
                dims_list = [
                    get_cluster_dimensions(self.cluster, self.pool, self.scheduler)
                ]
                if (
                    self.scheduler == "mesos"
                ):  # handle old (non-scheduler-aware) metrics
                    dims_list.insert(
                        0, get_cluster_dimensions(self.cluster, self.pool, None)
                    )
            else:
                dims_list = [{}]
...
def setup_signals_environment(pool: str, scheduler: str) -> Tuple[int, int]:
    ...
    app_signal_name = staticconf.read_string(
        "autoscale_signal.name", namespace=app_namespace, default=None,
    )
    if app_signal_name:
        signal_names.append(app_signal_name)
        signal_versions.append(
            staticconf.read_string(
                "autoscale_signal.branch_or_tag", namespace=app_namespace, default=pool,
            )
        )
        signal_namespaces.append(
            staticconf.read_string(
                "autoscale_signal.namespace", namespace=app_namespace, default=pool
            ),
        )
