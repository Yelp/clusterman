...
class EC2FleetResourceGroup(AWSResourceGroup):
    ...
    def modify_target_capacity(
        ...
        if not response["Return"]:
            logger.critical(
                "Could not change size of spot fleet:\n{resp}".format(
                    resp=json.dumps(response)
                )
            )
            raise ResourceGroupError(
                "Could not change size of spot fleet: check logs for details"
            )
    ...
    def instance_ids(self) -> Sequence[str]:
        ...
        return [
            instance["InstanceId"]
            for instance in ec2_describe_fleet_instances(self.group_id)
        ]
    ...
    def _generate_market_weights(self) -> Mapping[InstanceMarket, float]:
        ...
        for launch_template_config in self._configuration["LaunchTemplateConfigs"]:
            ...
            market_weights[get_market(instance_type, subnet_id)] = override[
                "WeightedCapacity"
            ]
