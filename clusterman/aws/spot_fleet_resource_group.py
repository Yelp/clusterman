...
SpotFleetResourceGroupConfig = TypedDict(
    "SpotFleetResourceGroupConfig", {"s3": _S3Config, "tag": str,}
)
...
class SpotFleetResourceGroup(AWSResourceGroup):
    ...
    def modify_target_capacity(
        self, target_capacity: float, *, dry_run: bool = False,
    ) -> None:
        if self.is_stale:
            logger.info(
                f"Not modifying spot fleet request since it is in state {self.status}"
            )
            return
        kwargs = {
    ...
    def instance_ids(self) -> Sequence[str]:
        ...
        return [
            instance["InstanceId"]
            for page in ec2.get_paginator("describe_spot_fleet_instances").paginate(
                SpotFleetRequestId=self.group_id
            )
        ...
        **kwargs: Any,
        ...
        if "s3" in config:
            s3_resource_groups = load_spot_fleets_from_s3(
                config["s3"]["bucket"], config["s3"]["prefix"], pool=pool,
            )
...
def load_spot_fleets_from_s3(
    bucket: str, prefix: str, pool: str = None
    ...
    for obj_metadata in object_list["Contents"]:
        ...
        for resource_key, resource in sfr_metadata[
