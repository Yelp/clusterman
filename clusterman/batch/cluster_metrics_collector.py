...
METRICS_TO_WRITE = [
    ...
    MetricToWrite(
        generate_simple_metadata, METADATA, aggregate_meteorite_dims=False, pools=All
    ),
    MetricToWrite(
...
class ClusterMetricsCollector(
    BatchDaemon, BatchLoggingMixin, BatchRunningSentinelMixin
):
        ...
        for scheduler, pools in self.pools.items():
            for pool in pools:
                self.config.watchers.append(
                    {
                        f"{pool}.{scheduler}": get_pool_config_path(
    ...
    def run(self) -> None:
        ...
        while self.running:
            time.sleep(
                splay_event_time(
                    self.run_interval, self.get_name() + self.options.cluster,
                )
            )
    ...
    def write_all_metrics(self) -> bool:
        ...
        for metric_to_write in METRICS_TO_WRITE:
            with self.metrics_client.get_writer(
                ...
                try:
                    self.write_metrics(
                        writer, metric_to_write.generator, metric_to_write.pools
                    )
                except socket.timeout:
                    # Try to get metrics for the rest of the clusters, but make sure we know this failed
                    logger.warn(
                        f"Timed out getting cluster metric data:\n\n{format_exc()}"
                    )
                    successful = False
                    continue
    ...
    def write_metrics(
        ...
        for pool, manager in self.pool_managers.items():
            ...
            for cluster_metric in metric_generator(manager):
                metric_name = generate_key_with_dimensions(
                    cluster_metric.metric_name, cluster_metric.dimensions
                )
                data = (metric_name, int(time.time()), cluster_metric.value)
                logger.info(
                    f"Writing value {cluster_metric.value} for metric {metric_name} to metric store"
                )
