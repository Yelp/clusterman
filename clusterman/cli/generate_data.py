...
def get_frequency_function(frequency_conf):
    ...
    if isinstance(frequency_conf, str):
        ...
        return lambda current_time: current_time.shift(
            seconds=int(gen_func(**frequency_conf["params"])) + 1
        )
...
def get_markets_and_values(dict_keys, values_func):
    ...
    return {
        k: values_func()
        for k in random.sample(dict_keys, random.randint(1, len(dict_keys)))
    }
