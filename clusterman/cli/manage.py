...
def get_target_capacity_value(target_capacity: str, pool: str, scheduler: str) -> int:
    ...
    if target_capacity == "min":
        return staticconf.read_int(
            "scaling_limits.min_capacity", namespace=pool_namespace
        )
    elif target_capacity == "max":
        return staticconf.read_int(
            "scaling_limits.max_capacity", namespace=pool_namespace
        )
    else:
...
def change_target_capacity(
    manager: PoolManager, target_capacity: str, dry_run: bool
) -> str:
    old_target = manager.target_capacity
    requested_target = get_target_capacity_value(
        target_capacity, manager.pool, manager.scheduler
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.target_capacity:
        log_message = change_target_capacity(
            manager, args.target_capacity, args.dry_run
        )
