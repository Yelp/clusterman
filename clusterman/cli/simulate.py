...
try:
    ...
    logger.warning(
        f"ImportError: {e}, unable to import code to make reports, some simulator commands will fail"
    )
...
def _load_metrics(metrics_data_files, pool):
    ...
    metrics_client = ClustermanMetricsSimulationClient(
        metrics, region_name=region_name, app_identifier=pool
    )
...
def _populate_cluster_size_events(simulator, start_time, end_time):
    ...
    for i, (timestamp, data) in enumerate(capacity_metrics["fulfilled_capacity"]):
        ...
        use_join_delay = (
            i != 0
        )  # Want to start the cluster out at the expected capacity
        simulator.add_event(
            ModifyClusterSizeEvent(arrow.get(timestamp), market_data, use_join_delay)
        )
...
def _populate_price_changes(simulator, start_time, end_time, discount):
    for market in simulator.markets:
        ...
        for timestamp, price in market_prices["spot_prices"]:
            ...
            simulator.add_event(
                InstancePriceChangeEvent(arrow.get(timestamp), {market: price},)
            )
...
def _run_simulation(args, metrics_client):
    ...
    simulator = Simulator(
        metadata, args.start_time, args.end_time, args.autoscaler_config, metrics_client
    )
    ...
    if args.compare:
        if len(args.compare) > 2:
            raise argparse.ArgumentError(
                None, f"Cannot compare more than two simulations: {args.compare}"
            )
