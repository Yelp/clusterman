...
def _write_resource_group_line(group) -> None:
    ...
    print(
        f"\t{group.id}: {status_str} ({group.fulfilled_capacity} / {group.target_capacity})"
    )
...
def _write_agent_details(node_metadata: ClusterNodeMetadata) -> None:
    ...
    if node_metadata.agent.state == AgentState.RUNNING:
        colored_resources = [
            ...
            for (allocated, total) in zip(
                node_metadata.agent.allocated_resources,
...
def _write_summary(manager: PoolManager) -> None:
    ...
    total_mem = format_size(
        manager.cluster_connector.get_resource_total("mem") * 1000000
    )
    total_disk = format_size(
        manager.cluster_connector.get_resource_total("disk") * 1000000
    )
    ...
    allocated_mem = format_size(
        manager.cluster_connector.get_resource_allocation("mem") * 1000000
    )
    allocated_disk = format_size(
        manager.cluster_connector.get_resource_allocation("disk") * 1000000
    )
    allocated_gpus = manager.cluster_connector.get_resource_allocation("gpus")
    print(
        f"\tCPU allocation: {allocated_cpus:.1f} CPUs allocated to tasks, {total_cpus:.1f} total"
    )
    print(
        f"\tMemory allocation: {allocated_mem} memory allocated to tasks, {total_mem} total"
    )
    print(
        f"\tDisk allocation: {allocated_disk} disk space allocated to tasks, {total_disk} total"
    )
    print(
        f"\tGPUs allocation: {allocated_gpus} GPUs allocated to tasks, {total_gpus} total"
    )
...
def print_status(manager: PoolManager, args) -> None:
    ...
    for group in manager.resource_groups.values():
        ...
        for metadata in node_metadatas:
            if (
                metadata.instance.group_id != group.id
                or (args.only_orphans and metadata.agent.state != AgentState.ORPHANED)
                or (args.only_idle and metadata.agent.state != AgentState.IDLE)
            ):
                continue
