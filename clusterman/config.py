...
def _load_module_configs(env_config_path: str):
    ...
    for config in staticconf.read_list("module_config", default=[]):
        ...
        staticconf.DictConfiguration(
            config.get("config", {}), namespace=config["namespace"]
        )
...
def setup_config(args: argparse.Namespace) -> None:
    ...
    cluster_config_directory = (
        getattr(args, "cluster_config_directory", None) or DEFAULT_CLUSTER_DIRECTORY
    )
    ...
    # different region.
        ...
        aws_region = staticconf.read_string(
            f"clusters.{cluster}.aws_region", default=None
        )
        if pool:
...
def load_cluster_pool_config(
    cluster: str, pool: str, scheduler: str, signals_branch_or_tag: Optional[str]
) -> None:
