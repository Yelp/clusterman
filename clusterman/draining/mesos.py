...
def load_credentials(mesos_secret_path):
    ...
    if not mesos_secret_path:
        ...
        try:
            ...
            log.error(
                f"maintenance calls must have valid credentials ({mesos_secret_path})"
            )
            raise
    try:
...
def drain(operator_client, hostnames, start, duration):
    ...
    payload = build_maintenance_schedule_payload(
        operator_client, hostnames, start, duration, drain=True
    )
    try:
...
def build_maintenance_schedule_payload(
    operator_client, hostnames, start=None, duration=None, drain=True
):
    ...
    """
    schedule = get_maintenance_schedule(operator_client).json()[
