...
class DrainingClient:
    def __init__(self, cluster_name: str) -> None:
        ...
        self.warning_queue_url = staticconf.read_string(
            f"clusters.{cluster_name}.warning_queue_url", default=None,
        )

    def submit_instance_for_draining(
        self, instance: InstanceMetadata, sender: Type[AWSResourceGroup]
    ) -> None:
    ...
    def submit_host_for_draining(self, host: Host) -> None:
        return self.client.send_message(
            QueueUrl=self.drain_queue_url,
            MessageAttributes={
                "Sender": {"DataType": "String", "StringValue": host.sender,},
            },
            MessageBody=json.dumps(
                {
    ...
    def submit_host_for_termination(
        self, host: Host, delay: Optional[int] = None
    ) -> None:
        delay_seconds = (
            delay
            if delay is not None
            else staticconf.read_int(
    ...
    def get_warned_host(self) -> Optional[Host]:
        ...
        if messages:
            ...
            if not host:
                logger.warning(
                    "Couldn't derive host data from instance id {} skipping".format(
                        event_data["detail"]["instance-id"]
                    )
                )
        ...
        if host_to_terminate:
            ...
            if host_to_terminate.hostname:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to down {hostname_ip} continuing to terminate anyway: {e}"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to up {hostname_ip} continuing to terminate anyway: {e}"
                    )
            else:
    ...
    def process_drain_queue(
        ...
        if (
            host_to_process
            and host_to_process.instance_id not in self.draining_host_ttl_cache
        ):
            self.draining_host_ttl_cache[
                host_to_process.instance_id
            ] = arrow.now().shift(seconds=DRAIN_CACHE_SECONDS)
    ...
    def process_warning_queue(self) -> None:
        ...
        if host_to_process:
            ...
            for pool in get_pool_name_list(
                self.cluster, "mesos"
            ):  # draining only supported for Mesos clusters
...
def host_from_instance_id(
    sender: str, receipt_handle: str, instance_id: str,
) -> Optional[Host]:
...
def process_queues(cluster_name: str) -> None:
    ...
    while True:
        ...
        draining_client.process_drain_queue(mesos_operator_client=operator_client,)
...
def main(args: argparse.Namespace) -> None:
    ...
    for pool in get_pool_name_list(args.cluster, "mesos"):
        load_cluster_pool_config(
            args.cluster, pool, "mesos", None
        )  # drainer only supported for mesos
    process_queues(args.cluster)


@subparser(
    "drain", "Drains and terminates instances submitted to SQS by clusterman", main
)
