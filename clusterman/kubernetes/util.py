...
DEFAULT_KUBERNETES_DISK_REQUEST = (
    "0"  # Kubernetes doesn't schedule based on disk allocation right now
)
...
class ResourceParser:
    ...
    def mem(resources):
        resources = resources or {}
        return (
            parse_size(resources.get("memory", DEFAULT_KUBERNETES_MEMORY_REQUEST))
            / 1000000
        )
...
def allocated_node_resources(pods: List[KubernetesPod]) -> ClustermanResources:
    ...
    for pod in pods:
        ...
        gpus += sum(
            ResourceParser.gpus(c.resources.requests) for c in pod.spec.containers
        )
