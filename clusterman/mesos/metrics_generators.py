...
SYSTEM_METRICS = {
    "cpus_allocated": lambda manager: manager.cluster_connector.get_resource_allocation(
        "cpus"
    ),
    "mem_allocated": lambda manager: manager.cluster_connector.get_resource_allocation(
        "mem"
    ),
    "disk_allocated": lambda manager: manager.cluster_connector.get_resource_allocation(
        "disk"
    ),
...
SIMPLE_METADATA = {
    ...
    "fulfilled_capacity": lambda manager: {
        str(market): value for market, value in manager.get_market_capacities().items()
    },
...
def generate_system_metrics(
    ...
    dimensions = get_cluster_dimensions(
        manager.cluster, manager.pool, manager.scheduler
    )
...
def _prune_resources_dict(resources_dict: Mapping) -> MutableMapping:
    return {
        resource: resources_dict[resource]
        for resource in ("cpus", "mem", "disk", "gpus")
    }


def _get_framework_metadata_for_frameworks(
    cluster_connector: MesosClusterConnector, framework_state: FrameworkState,
) -> Generator[ClusterMetric, None, None]:
...
def generate_framework_metadata(
    manager: PoolManager,
) -> Generator[ClusterMetric, None, None]:
    yield from _get_framework_metadata_for_frameworks(
        cast(MesosClusterConnector, manager.cluster_connector), FrameworkState.RUNNING,
    )
    yield from _get_framework_metadata_for_frameworks(
