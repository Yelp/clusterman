...
class SimulatedPoolManager(PoolManager):
    def __init__(
        ...
        self.pool_config = staticconf.NamespaceReaders(
            POOL_NAMESPACE.format(pool=self.pool, scheduler="mesos")
        )
        ...
        self.cluster_connector = SimulatedClusterConnector(
            self.cluster, self.pool, self.simulator
        )
    ...
    def get_node_metadatas(
        self, aws_state_filter: Optional[Collection[str]] = None,
