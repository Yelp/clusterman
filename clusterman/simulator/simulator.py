...
class Simulator:
    def __init__(
        ...
        self.instance_prices: Mapping[
            InstanceMarket, PiecewiseConstantFunction
        ] = defaultdict(lambda: PiecewiseConstantFunction())
    ...
    def add_event(self, evt):
        ...
        if evt.time >= self.end_time:
            logger.info(
                f"Adding event after simulation end time ({evt.time}); event ignored"
            )
            return
        elif evt.time < self.current_time:
            logger.info(
                f"Adding event before self.current_time ({evt.time}); event ignored"
            )
            return
    ...
    def run(self):
        ...
        print(
            "Simulation complete ({time}s)".format(
    ...
    def get_data(
        ...
        if key == "cpus":
            ...
            return self.cost_per_hour.integrals(
                start_time, end_time, step, transform=hour_transform
            )
            ...
            percent_unallocated = (
                self.aws_cpus - self.mesos_cpus_allocated
            ) / self.aws_cpus
            percent_cost = percent_unallocated * self.cost_per_hour
            return percent_cost.integrals(
                start_time, end_time, step, transform=hour_transform
            )
            ...
            max_fn = piecewise_max(
                self.mesos_cpus_allocated - self.aws_cpus, PiecewiseConstantFunction()
            )
    ...
    def _compute_instance_cost(self, instance):
        ...
        for bp_timestamp in piecewise_breakpoint_generator(
            prices.breakpoints, instance.start_time, instance.end_time
        ):
            ...
            if bp_timestamp >= curr_timestamp + self.billing_frequency:
                self.cost_per_hour.add_delta(
                    curr_timestamp + self.billing_frequency, delta
                )
                ...
                jumps, remainder = divmod(
                    bp_timestamp - curr_timestamp, self.billing_frequency
                )
                if not remainder:
                    jumps -= 1
        ...
        if (
            not instance.spot
            or not self.refund_outbid
            or instance.bid_price > prices.call(instance.end_time)
        ):
    ...
    def _make_autoscaler(self, autoscaler_config_file: str) -> None:
        ...
        for i in range(fetch_count):
            subprocess.run(
                ["fetch_clusterman_signal", str(i), signal_dir], check=True, env=env
            )
        ...
        pool_manager = SimulatedPoolManager(
            self.metadata.cluster, self.metadata.pool, configs, self
        )
        metric_values = self.metrics_client.get_metric_values(
            ...
            extra_dimensions=get_cluster_dimensions(
                self.metadata.cluster, self.metadata.pool, self.metadata.scheduler
            ),
        )
        ...
        with patch_join_delay():
            ...
            pool_manager.modify_target_capacity(
                actual_target_capacity, force=True, prune=False
            )
...
def _make_comparison_sim(sim1, sim2, op, opcode):
    metadata = SimulationMetadata(
        ...
        f"{sim1.metadata.cluster}"
        if sim1.metadata.cluster == sim2.metadata.cluster
        else f"{sim1.metadata.cluster}, {sim2.metadata.cluster}",
        f"{sim1.metadata.pool}"
        if sim1.metadata.pool == sim2.metadata.pool
        else f"{sim1.metadata.pool}, {sim2.metadata.pool}",
    )
    if sim1.start_time != sim2.start_time or sim1.end_time != sim2.end_time:
        logger.warn(
            "Compared simulators do not have the same time boundaries; "
