    ...
    with open(args.mapping_file) as f:
        for line in f.readlines():
            ...
            for page in query.paginate(
                ...
                request_item_list = [
                    {"PutRequest": {"Item": item}} for item in page["Items"]
                ] + [
                    {
                        "DeleteRequest": {
                            "Key": {"key": {"S": old}, "timestamp": item["timestamp"]}
                        }
                    }
                    for item in page["Items"]
                ]
                for i in range(0, len(request_item_list), BATCH_WRITE_SIZE):
                    request_items = {
                        table_name: request_item_list[
                            i : min(i + BATCH_WRITE_SIZE, len(request_item_list))
                        ]
                    }
