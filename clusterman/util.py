...
def sensu_checkin(
    ...
    **kwargs: Any,
    ...
    pool_namespace = (
        POOL_NAMESPACE.format(pool=app, scheduler=scheduler) if app else "no-namespace"
    )
    ...
    #
    sensu_config = dict(
        staticconf.read_list(
            "sensu_config", default=[{}], namespace=pool_namespace
        ).pop()
    )
    if not sensu_config:
        sensu_config = dict(
            staticconf.read_list(f"clusters.{source}.sensu_config", default=[{}]).pop()
        )
    if not sensu_config:
...
def read_int_or_inf(reader, param):
    return (
        float("inf")
        if reader.read_string(param, default=0) == "inf"
        else reader.read_int(param, default=0)
    )
...
def get_pool_name_list(cluster_name: str, scheduler: str) -> List[str]:
    ...
    return [
        ...
        for f in os.listdir(cluster_config_directory)
        if f[0] != "."
        and f.endswith(scheduler)  # skip dotfiles and only read scheduler files
    ]
...
def get_cluster_dimensions(
    cluster: str, pool: str, scheduler: Optional[str]
) -> Dict[str, str]:
    return {
        "cluster": cluster,
