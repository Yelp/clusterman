...
RESERVED_KEYS = frozenset(
    ["key", "timestamp", "expiration_timestamp", GSI_PK, GSI_SORT]
)
...
class ClustermanMetricsBotoClient(object):
    ...
    def __init__(
        ...
        ttl_days = ttl_days or staticconf.read_int(
            "dynamodb.ttl_days", namespace=CONFIG_NAMESPACE
        )
    ...
    def get_metric_values(
        ...
        if metric_type != APP_METRICS:
            if is_regex:
                raise ValueError(
                    "regex expressions are not supported for {type}".format(
                        type=metric_type
                    )
                )
            app_identifier = ""
        ...
        query_with_dims = key_prefix + generate_key_with_dimensions(
            metric_query, extra_dimensions
        )

        cached_data: MetricsValuesDict = defaultdict(list)
        if use_cache:
            cached_data, time_start, time_end = self._cache_lookup(
                query_with_dims, time_start, time_end
            )

        new_data: MetricsValuesDict = defaultdict(list)
        if (
            time_start < time_end
        ):  # Skip querying dynamodb if all the metrics were in the cache
        ...
        for metric_key in set(cached_data) | set(new_data):
            ...
            metric_key_without_app = (
                metric_key[len(app_identifier) + 1 :]
                if metric_type == APP_METRICS
                else metric_key
            )
    ...
    def _get_new_metric_values(
        ...
        if is_regex:
            metric_keys = self._get_keys_from_query(
                key_prefix, metric_query, time_start, time_end, table
            )
        else:
        ...
        for metric_key in metric_keys:
            ...
            query_condition = Key("key").eq(full_query_key) & Key("timestamp").between(
                time_start, time_end
            )
            response = table.query(KeyConditionExpression=query_condition)

            new_data[metric_key] = self._extract_timestamp_and_value_from_items(
                response["Items"]
            )
            ...
            while response.get("LastEvaluatedKey") is not None:
                ...
                new_data[metric_key].extend(
                    self._extract_timestamp_and_value_from_items(response["Items"])
                )
    ...
    def _get_table_name(self, metric_type):
        if metric_type not in METRIC_TYPES:
            raise ValueError(
                "Metric type must be one of {types}".format(types=METRIC_TYPES)
            )

        return "{prefix}_{metric_type}".format(
            prefix=CLUSTERMAN_NAME, metric_type=metric_type
        )

    def _get_key_prefix(
        self, metric_type: str, app_identifier: Optional[str] = None
    ) -> str:
        app_identifier = app_identifier or self.app_identifier
        if metric_type == APP_METRICS:
    ...
    def _get_keys_from_query(
        ...
        for i in range(_GSI_PARTITIONS):
            ...
            response = table.query(
                IndexName=GSI_NAME, KeyConditionExpression=query_condition,
            )
            metric_keys |= {
                item["key"]
    ...
    def _cache_lookup(
        self, metric_query: str, time_start: int, time_end: int
    ) -> Tuple[MetricsValuesDict, int, int]:
        ...
        *could* be cases where writes to the datastore haven't completed when we read the cache, but this will only
        ...
        if time_start >= time_end:
            ...
            centry.data = defaultdict(
                list, {metric_name: ts for metric_name, ts in centry.data.items() if ts}
            )
    ...
    def _cache_store(
        self, metric_query: str, time_start: int, data: MetricsValuesDict
    ) -> None:
        ...
        for metric_name, ts in data.items():
            cache_end = bisect(
                ts, (five_minutes_ago, float("-inf"))
            )  # Don't cache the element exactly 5 minutes ago
            ...
            if ts:
                ...
                if (
                    len(centry.data[metric_name]) > 0
                    and ts[0][0] <= centry.data[metric_name][-1][0]
                ):
                    logger.warning(
        ...
        if should_invalidate:
            ...
            logger.info(
                "Caching entries for {key} between {s} and {t}".format(
                    key=metric_query, s=min_time, t=max_time,
                )
            )
