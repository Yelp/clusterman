...
from clusterman_metrics.boto_client import (
    MetricsValuesDict,
)  # noqa (just used for type-checking)
    ...
    if not set(obj).issubset(METRIC_TYPES):
        raise ValueError(
            "Invalid metric type in {gen}; valid choices are {valid}".format(
                gen=list(obj), valid=list(METRIC_TYPES),
            )
        )
    ...
    for metric in obj.values():
        for key in metric:
            ...
            if len(set(timestamps)) != len(timestamps):
                raise ValueError(
                    "Duplicate timestamps detected in {key}".format(key=metric[key])
                )
            if any(
                [not isinstance(timestamp, Number) for timestamp, __ in metric[key]]
            ):
                raise ValueError(
                    "Invalid timestamp values for {key}".format(key=metric[key])
                )
            if any(
                [
                    not isinstance(value, Number) and not isinstance(value, dict)
                    for __, value in metric[key]
                ]
            ):
                raise ValueError(
                    "Invalid metric values for {key}".format(key=metric[key])
                )
...
class ClustermanMetricsSimulationClient(ClustermanMetricsBotoClient):
    ...
    def get_writer(*args, **kwargs):
        def log_values(*args, **kwargs):
            while True:
                values = yield
                logger.warning(
                    "Client is read-only; not writing {values}".format(values=values)
                )
    ...
    def _get_new_metric_values(
        ...
        for metric_key in generated_keys:
            ...
            try:
                full_timeseries = self.generated_metrics.get(metric_type, {})[
                    full_query_key
                ]
            except KeyError:
                continue
            ...
            if (
                len(full_timeseries) > end_index
                and full_timeseries[end_index][0] == time_end
            ):
                end_index += 1
        ...
        if metrics:
            ...
            return super(
                ClustermanMetricsSimulationClient, self
            )._get_new_metric_values(
                key_prefix,
                metric_query,
                metric_type,
                time_start,
