...
def _get_resource_cost(resource, cluster, pool):
    default_cost = config_reader.read_float("cost_per_hour.defaults.{}".format(resource), default=0,)
    return config_reader.read_float("cost_per_hour.{}.{}.{}".format(cluster, pool, resource), default=default_cost,)


def should_warn(cost):
    threshold = config_reader.read_float("cost_warning_threshold", default=100,)
