...
@behave.given(
    "a pool manager with (?P<num>\d+) (?P<rg_type>asg|sfr|fleet) resource groups?"
)
...
@behave.given(
    "the fulfilled capacity of resource group (?P<rg_index>\d+) is (?P<capacity>\d+)"
)
def external_target_capacity(context, rg_index, capacity):
    ...
    with mock.patch(
        "clusterman.autoscaler.pool_manager.PoolManager._reload_resource_groups"
    ):
...
def broken_resource_group(context, rgid):
    ...
    rg.modify_target_capacity = mock.Mock(
        side_effect=ResourceGroupError("resource group is broken")
    )
...
@behave.then(
    "the(?P<remaining> remaining)? resource groups should have evenly-balanced capacity"
)
def check_target_capacity(context, remaining):
    target_capacity = 0
    if remaining:
        desired_capacity = (
            context.pool_manager.target_capacity - context.original_capacities[0]
        ) / (len(context.rg_ids) - 1)
    else:
...
def target_capacity_equals(context, rgid, capacity):
    assert_that(
        len(
            context.pool_manager.resource_groups[
                context.rg_ids[int(rgid) - 1]
            ].instance_ids
        ),
