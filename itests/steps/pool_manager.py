        ...
        if context.rg_type == "asg":
            if getattr(context, "reload", False):
                mock_asg_load.return_value = {
                    f"fake-asg-{i}": AutoScalingResourceGroup(f"fake-asg-{i}") for i in range(context.rg_num)
                }
            else:
...
def check_target_capacity(context, remaining):
    target_capacity = 0
    if remaining:
        desired_capacity = (context.pool_manager.target_capacity - context.original_capacities[0]) / (
