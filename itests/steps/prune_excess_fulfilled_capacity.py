...
def killed_tasks(context, max_tasks_to_kill):
    context.aws_pool_manager.max_tasks_to_kill = int(max_tasks_to_kill)
...
def killable_instance_with_weight(context, weight):
    context.aws_pool_manager.resource_groups[context.rg_ids[0]].market_weight = mock.Mock(return_value=int(weight))
    context.aws_pool_manager._get_prioritized_killable_instances = mock.Mock(return_value=[
...
def prune_excess_fulfilled_capacity(context, target):
    context.original_agents = context.aws_pool_manager.get_instance_metadatas()
    with mock.patch(
        ...
        context.aws_pool_manager.prune_excess_fulfilled_capacity(new_target_capacity=int(target))


@behave.then('(?P<num>\d+) instances? should be killed')
def check_no_instances_killed(context, num):
    assert_that(
        len(context.original_agents) - len(context.aws_pool_manager.get_instance_metadatas()),
        equal_to(int(num)),
    )
...
def check_killed_instance_group(context, rg_index):
    killed_agents = [
        ...
        if a.instance_id not in [m.instance_id for m in context.aws_pool_manager.get_instance_metadatas()]
    ]
    assert_that(
        killed_agents,
