    ...
    def mock_describe_sfrs(SpotFleetRequestIds):
        return {
            "SpotFleetRequestConfigs": [
                c for c in response["SpotFleetRequestConfigs"] if c["SpotFleetRequestId"] in SpotFleetRequestIds
            ]
        }
...
def check_n_instances_killed(context, num):
    ...
    context.killed_nodes = [
        n for n in context.original_agents if n.instance.instance_id not in [i["InstanceId"] for i in running_instances]
    ]
...
def check_killed_instances_stale(context):
    assert_that([n.instance.is_stale for n in context.killed_nodes], only_contains(True))
