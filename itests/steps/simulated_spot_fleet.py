...
def sfrg(context):
    ...
    with mock.patch(
        ...
        context.spot_fleet = SimulatedSpotFleetResourceGroup(
            spot_fleet_request_config, _make_mock_simulator()
        )
...
def no_capacity_in_market(context):
    ...
    context.spot_fleet.simulator.instance_prices[
        _MARKETS[context.outbid_market]
    ].add_breakpoint(arrow.get(300), 3.0)
    context.spot_fleet.simulator.current_time = arrow.get(480)
    terminate_ids = list(
        context.spot_fleet.instance_ids_by_market[_MARKETS[context.outbid_market]]
    )
...
def check_diversification(context):
    for market in _MARKETS:
        assert_that(
            context.spot_fleet.market_size(market)
            * (context.spot_fleet._instance_types[market].weight),
