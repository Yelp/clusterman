...
@behave.given(
    "market (?P<market_id>[a-cA-C]) has (?P<count>\d+) instances? at time (?P<time>\d+)"
)
...
@behave.given(
    "market (?P<market_id>[a-cA-C]) costs \$(?P<cost>\d+(?:\.\d+)?)/hour at time (?P<time>\d+)"
)
...
@behave.when(
    "the simulator runs for (?P<hours>\d+) hours(?P<per_second_billing> and billing is per-second)?"
)
def run_simulator(context, hours, per_second_billing):
    billing_frequency = (
        timedelta(seconds=1) if per_second_billing else timedelta(hours=1)
    )
    ...
    with staticconf.testing.PatchConfiguration(
        ...
        for market_id, prices in getattr(context, "markets", {}).items():
            for time, cost in sorted(prices):
                ...
                context.simulator.add_event(
                    InstancePriceChangeEvent(arrow.get(time), {market: cost})
                )
...
def check_cluster_cpus(context):
    assert_that(
        ...
        contains((arrow.get(300), 32), (arrow.get(1800), 0),),
    )
