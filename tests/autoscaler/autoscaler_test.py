...
def mock_autoscaler():
    ...
    with mock.patch(
        "clusterman.autoscaler.autoscaler.ClustermanMetricsBotoClient", autospec=True,
    ), mock.patch(
        ...
        mock_autoscaler = Autoscaler(
            "mesos-test", "bar", "mesos", ["bar"], monitoring_enabled=False
        )
...
def test_autoscaler_init_too_many_apps():
    with pytest.raises(NotImplementedError):
        Autoscaler(
            "mesos-test", "bar", "mesos", ["app1", "app2"], monitoring_enabled=False
        )
...
def test_monitoring_enabled(mock_signal, mock_autoscaler, monitoring_enabled):
    ...
    with mock.patch("clusterman.util._get_sensu") as mock_get_sensu:
        ...
        assert mock_get_sensu.return_value.send_event.call_count == (
...
def test_get_signal_for_app(mock_autoscaler, signal_response):
    with mock.patch(
        ...
        "clusterman.autoscaler.autoscaler.sensu_checkin"
    ) as mock_sensu:
    ...
    assert signal == (
        mock_autoscaler.default_signal
...
def test_autoscaler_run(dry_run, mock_autoscaler, run_timestamp):
    ...
    assert mock_autoscaler._compute_target_capacity.call_args == mock.call(
        {"cpus": 100000}
    )
...
def test_is_paused_with_expiration_timestamp(
    exp_timestamp, mock_autoscaler, run_timestamp
):
    with mock.patch("clusterman.autoscaler.autoscaler.dynamodb") as mock_dynamo:
        ...
        if exp_timestamp:
            mock_dynamo.get_item.return_value["Item"]["expiration_timestamp"] = {
                "N": exp_timestamp
            }
        assert mock_autoscaler._is_paused(run_timestamp) == (
            not exp_timestamp or int(exp_timestamp) > run_timestamp.timestamp
        )
...
class TestComputeTargetCapacity:
    ...
    @pytest.mark.parametrize(
        "signal_resource,total_resource,expected_capacity",
        [
            (None, 1000, 125),
    ...
    def test_single_resource(
        ...
        mock_autoscaler.pool_manager.cluster_connector.get_resource_total.return_value = (
            total_resource
        )
    ...
    def test_current_target_capacity_with_historical_data(self, mock_autoscaler):
        ...
        mock_autoscaler._get_historical_weighted_resource_value = mock.Mock(
            return_value=ClustermanResources(cpus=2, mem=26, disk=0, gpus=0)
        )
    ...
    def test_scale_most_constrained_resource(self, mock_autoscaler):
        ...
        mock_autoscaler.pool_manager.cluster_connector.get_resource_total.side_effect = (
...
def test_get_historical_weighted_resource_value_no_historical_data(mock_autoscaler):
    ...
    assert (
        mock_autoscaler._get_historical_weighted_resource_value()
        == ClustermanResources()
    )
...
def test_get_smoothed_non_zero_metadata(mock_autoscaler):
    ...
    assert mock_autoscaler._get_smoothed_non_zero_metadata(
        "some_metric", 0, 200, smoothing=3
    ) == (120, 140, (40 + 23 + 41) / 3,)
...
def test_get_smoothed_non_zero_metadata_no_data(mock_autoscaler):
    ...
    assert (
        mock_autoscaler._get_smoothed_non_zero_metadata(
            "some_metric", 0, 200, smoothing=3
        )
        is None
    )
