...
from clusterman.autoscaler.autoscaler_v2 import DEFAULT_SIGNAL_ROLE
from clusterman.autoscaler.autoscaler_v2 import DELTA_GAUGE_NAME
...
def mock_logger():
    with mock.patch('clusterman.autoscaler.autoscaler_v2.logger') as mock_logger:
        yield mock_logger
...
def mock_import_signals():
    with mock.patch('clusterman.autoscaler.autoscaler_v2.import_module') as mock_import:
        yield mock_import


@pytest.fixture
def mock_role_manager():
    with mock.patch('clusterman.autoscaler.autoscaler_v2.MesosRoleManager', autospec=True) as mock_role_manager:
        yield mock_role_manager


@pytest.fixture
@mock.patch('clusterman.autoscaler.autoscaler_v2.AutoscalerV2.load_signal', autospec=True)
def mock_autoscaler(mock_load_signal, mock_role_manager, mock_gauge):
    mock_autoscaler = AutoscalerV2('foo', 'bar')
...
def mock_constrain_delta():
    with mock.patch('clusterman.autoscaler.autoscaler_v2.AutoscalerV2._constrain_cluster_delta', autospec=True) as mock_constrain:
...
def test_autoscaler_init(mock_load_signal, mock_role_manager, mock_gauge):
    ...
    assert mock_gauge.call_args_list == [mock.call(DELTA_GAUGE_NAME, {'cluster': 'foo', 'role': 'bar', 'version': 'v2'})]
...
@mock.patch('clusterman.autoscaler.autoscaler_v2.read_signal_config', autospec=True)
@mock.patch('clusterman.autoscaler.autoscaler_v2.AutoscalerV2._init_signal_from_config', autospec=True)
