...
def test_modify_target_capacity_skip_failing_group(mock_pool_manager):
    list(mock_pool_manager.resource_groups.values())[
        0
    ].modify_target_capacity.side_effect = ResourceGroupError("foo")
    with mock.patch(
        "clusterman.autoscaler.pool_manager.get_monitoring_client"
    ) as mock_monitoring_client:
        mock_pool_manager.modify_target_capacity(1234)
...
def test_modify_target_capacity(new_target, constrained_target, mock_pool_manager):
    ...
    mock_pool_manager._constrain_target_capacity = mock.Mock(
        return_value=constrained_target
    )
    mock_pool_manager._compute_new_resource_group_targets = mock.Mock(
        return_value={f"sfr-{i}": i for i in range(7)}
    )
...
class TestPruneExcessFulfilledCapacity:
    ...
    def mock_pool_manager(self, mock_pool_manager, mock_nodes_to_prune):
        mock_pool_manager._choose_nodes_to_prune = mock.Mock(
            return_value=mock_nodes_to_prune
        )
    ...
    def test_dry_run(self, mock_pool_manager):
        ...
        assert (
            mock_pool_manager.draining_client.submit_instance_for_draining.call_count
            == 0
        )
    ...
    def test_drain_queue(self, mock_pool_manager, mock_nodes_to_prune):
        ...
        assert (
            mock_pool_manager.draining_client.submit_instance_for_draining.call_args_list
            == [
                mock.call(
                    mock_nodes_to_prune["sfr-1"][0].instance, sender=AWSResourceGroup
                ),
                mock.call(
                    mock_nodes_to_prune["sfr-3"][0].instance, sender=AWSResourceGroup
                ),
                mock.call(
                    mock_nodes_to_prune["sfr-3"][1].instance, sender=AWSResourceGroup
                ),
                mock.call(
                    mock_nodes_to_prune["sfr-3"][2].instance, sender=AWSResourceGroup
                ),
            ]
        )
    ...
    def test_terminate_immediately(self, mock_pool_manager):
        mock_pool_manager.prune_excess_fulfilled_capacity(100)
        assert mock_pool_manager.resource_groups[
            "sfr-1"
        ].terminate_instances_by_id.call_args == mock.call([1])
        assert mock_pool_manager.resource_groups[
            "sfr-3"
        ].terminate_instances_by_id.call_args == mock.call([4, 5, 6])
...
class TestReloadResourceGroups:
    def test_malformed_config(self, mock_logger, mock_pool_manager):
        with staticconf.testing.MockConfiguration(
            ...
            mock_pool_manager.pool_config = staticconf.NamespaceReaders(
                "bar.mesos_config"
            )
...
class TestConstrainTargetCapacity:
    def test_positive_delta(self, mock_logger, force, mock_pool_manager):
        ...
        assert mock_pool_manager._constrain_target_capacity(1000, force) == (
            1000 if force else 249
        )
        mock_pool_manager.max_capacity = 97
        assert mock_pool_manager._constrain_target_capacity(1000, force) == (
            1000 if force else 97
        )
    ...
    def test_negative_delta(self, mock_logger, force, mock_pool_manager):
        ...
        assert mock_pool_manager._constrain_target_capacity(20, force) == (
            20 if force else 39
        )
...
class TestChooseNodesToPrune:
    ...
    def test_killable_node_too_many_tasks(self, mock_logger, mock_pool_manager):
        mock_pool_manager._get_prioritized_killable_nodes = mock.Mock(
            return_value=[_make_metadata("sfr-1", "i-1")]
        )
        assert mock_pool_manager._choose_nodes_to_prune(100, None) == {}
        assert (
            "would take us over our max_tasks_to_kill"
    ...
    def test_killable_nodes_under_target_capacity(self, mock_logger, mock_pool_manager):
        ...
        assert (
            "would take us under our target_capacity"
            in mock_logger.info.call_args[0][0]
        )
    ...
    def test_kill_node(self, mock_logger, mock_pool_manager):
        ...
        mock_pool_manager.max_tasks_to_kill = 100
        assert (
            mock_pool_manager._choose_nodes_to_prune(100, None)["sfr-1"][
                0
...
def test_compute_new_resource_group_targets_no_unfilled_capacity(mock_pool_manager):
    ...
    assert sorted(
        list(
            mock_pool_manager._compute_new_resource_group_targets(
                target_capacity
            ).values()
        )
    ) == [
...
@pytest.mark.parametrize("orig_targets", [10, 17])
def test_compute_new_resource_group_targets_all_equal_with_remainder(
    orig_targets, mock_pool_manager
):
    for group in mock_pool_manager.resource_groups.values():
        group.target_capacity = orig_targets
...
def test_compute_new_resource_group_targets_scale_up_stale_pools_0(
    non_stale_capacity, mock_pool_manager
):
    for group in list(mock_pool_manager.resource_groups.values())[:3]:
...
def test_instance_kill_order(mock_pool_manager):
    ...
    killable_instance_ids = [
        node_metadata.instance.instance_id for node_metadata in killable_nodes
    ]
