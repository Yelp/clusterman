...
@pytest.fixture
def mock_asg_config(
    mock_subnet, mock_launch_config, mock_asg_name, mock_cluster, mock_pool
):
    asg = {
        ...
        "Tags": [
            {
                ...
                "Value": json.dumps(
                    {"pool": mock_pool, "paasta_cluster": mock_cluster}
                ),
            },
...
def test_group_config(mock_asg_config):
    ...
    assert (
        group_config["AutoScalingGroupName"] == mock_asg_config["AutoScalingGroupName"]
    )
...
def test_launch_config_retry(mock_asrg, mock_launch_config):
    ...
    mock_describe_launch_configs = mock.Mock(side_effect=[no_configs, good_configs,])
...
def test_modify_target_capacity_down(mock_asrg, stale_instances):
    ...
    with mock.patch(
        ...
        assert mock_asrg.fulfilled_capacity == max(
            old_target_capacity, new_target_capacity + stale_instances
        )
...
def test_status(mock_asrg, stale_instances):
    ...
    with mock.patch(
        ...
        new_callable=mock.PropertyMock(
            return_value=mock_asrg.instance_ids[:stale_instances]
        ),
    ):
