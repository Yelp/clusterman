        ...
        return {
            "sfr-123": {
                ...
                "puppet:role::paasta": json.dumps(
                    {"pool": "default", "paasta_cluster": "westeros-prod",}
                ),
            },
            "sfr-456": {
                "some": "tag",
                "paasta": "true",
                "puppet:role::paasta": json.dumps(
                    {"pool": "another", "paasta_cluster": "westeros-prod",}
                ),
            },
            "sfr-789": {
                "some": "tag",
...
def test_terminate_instance_missing_subnet(mock_logger, mock_resource_group):
    ...
    with mock.patch(
        ...
        assert not mock_resource_group.terminate_instances_by_id(
...
def test_get_node_metadatas(mock_resource_group):
    ...
    with mock.patch("clusterman.aws.aws_resource_group.gethostbyaddr") as mock_get_host:
        mock_get_host.side_effect = lambda ip: {
            ips[i]: (f"host{i}",) for i in range(5)
        }[ip]
    ...
    for i, instance_metadata in enumerate(instance_metadatas):
        ...
        assert (
            ...
            == mock_resource_group.instances[i]["InstanceId"]
        )
