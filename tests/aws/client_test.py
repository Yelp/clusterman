...
def test_empty_filters():
    ...
    with mock.patch('clusterman.aws.client.ec2.get_paginator') as mock_paginator:
        ...
        mock_paginator.assert_has_calls([call().paginate(Filters=[], InstanceIds=ids)])
    ...
    with mock.patch('clusterman.aws.client.ec2.get_paginator') as mock_paginator:
        ...
        for i in range(target_call_count):
            mock_paginator.assert_has_calls(
                [call().paginate(Filters=[{'Values': fake_values[i * FILTER_LIMIT:(i + 1) * FILTER_LIMIT]}], InstanceIds=[])])
