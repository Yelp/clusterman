...
def test_load_spot_fleets_from_s3():
    ...
    s3.put_object(
        ...
        Key="fake-region/sfr-1.json",
        Body=json.dumps(
            {"cluster_autoscaling_resources": {"aws_spot_fleet_request": {"id": "sfr-1", "pool": "my-pool"}}}
        ).encode(),
    )
    s3.put_object(
        Bucket="fake-clusterman-sfrs",
        Key="fake-region/sfr-2.json",
        Body=json.dumps(
            {"cluster_autoscaling_resources": {"aws_spot_fleet_request": {"id": "sfr-2", "pool": "my-pool"}}}
        ).encode(),
    )
    s3.put_object(
        Bucket="fake-clusterman-sfrs",
        Key="fake-region/sfr-3.json",
        Body=json.dumps(
            {"cluster_autoscaling_resources": {"aws_spot_fleet_request": {"id": "sfr-3", "pool": "not-my-pool"}}}
        ).encode(),
    )
...
def test_load_spot_fleets():
    with mock.patch("clusterman.aws.spot_fleet_resource_group.AWSResourceGroup.load",) as mock_tag_load, mock.patch(
        "clusterman.aws.spot_fleet_resource_group.load_spot_fleets_from_s3",
    ) as mock_s3_load:
        mock_tag_load.return_value = {
            "sfr-1": mock.Mock(id="sfr-1"),
            "sfr-2": mock.Mock(id="sfr-2"),
        }
        mock_s3_load.return_value = {
            "sfr-3": mock.Mock(id="sfr-3", status="cancelled"),
            "sfr-4": mock.Mock(id="sfr-4"),
        }
...
def test_modify_target_capacity_up(mock_spot_fleet_resource_group):
    mock_spot_fleet_resource_group.modify_target_capacity(20)
    assert (
        ec2.describe_spot_fleet_requests(SpotFleetRequestIds=[mock_spot_fleet_resource_group.group_id],)[
            "SpotFleetRequestConfigs"
        ][0]["SpotFleetRequestConfig"]["TargetCapacity"]
        == 20
    )
    assert (
        len(
...
def test_modify_target_capacity_down(mock_spot_fleet_resource_group):
    ...
    new_config = ec2.describe_spot_fleet_requests(SpotFleetRequestIds=[mock_spot_fleet_resource_group.group_id],)[
        "SpotFleetRequestConfigs"
    ][0]["SpotFleetRequestConfig"]
...
def test_modify_target_capacity_error(mock_spot_fleet_resource_group):
    with mock.patch(
        "clusterman.aws.spot_fleet_resource_group.ec2.modify_spot_fleet_request"
    ) as mock_modify, pytest.raises(ResourceGroupError):
