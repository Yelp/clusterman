...
def test_load_spot_fleets_from_s3():
        ...
        Body=json.dumps(
            {
                "cluster_autoscaling_resources": {
                    "aws_spot_fleet_request": {"id": "sfr-1", "pool": "my-pool"}
                }
            }
        ).encode(),
    )
    s3.put_object(
        ...
        Body=json.dumps(
            {
                "cluster_autoscaling_resources": {
                    "aws_spot_fleet_request": {"id": "sfr-2", "pool": "my-pool"}
                }
            }
        ).encode(),
    )
    s3.put_object(
        ...
        Body=json.dumps(
            {
                "cluster_autoscaling_resources": {
                    "aws_spot_fleet_request": {"id": "sfr-3", "pool": "not-my-pool"}
                }
            }
        ).encode(),
    )
    ...
    with mock.patch("clusterman.aws.spot_fleet_resource_group.SpotFleetResourceGroup",):
        sfrgs = load_spot_fleets_from_s3(
            bucket="fake-clusterman-sfrs", prefix="fake-region", pool="my-pool",
        )
        assert len(sfrgs) == 2
    ...
    with mock.patch(
        ...
        spot_fleets = SpotFleetResourceGroup.load(
            ...
            config={
                ...
                "s3": {"bucket": "fake-clusterman-sfrs", "prefix": "fake-region",},
            },
        )
...
def test_modify_target_capacity_stale(mock_spot_fleet_resource_group):
    with mock.patch(
        ...
        mock_describe.return_value = {
            "SpotFleetRequestConfigs": [{"SpotFleetRequestState": "cancelled_running"}],
        }
...
def test_is_stale_not_found(mock_spot_fleet_resource_group):
    with mock.patch(
        ...
        mock_describe.side_effect = botocore.exceptions.ClientError(
            {"Error": {"Code": "InvalidSpotFleetRequestId.NotFound"}}, "foo",
        )
...
def test_is_stale_error(mock_spot_fleet_resource_group):
    with mock.patch(
        "clusterman.aws.spot_fleet_resource_group.ec2.describe_spot_fleet_requests"
    ) as mock_describe, pytest.raises(botocore.exceptions.ClientError):
