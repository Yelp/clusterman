...
def mock_get_data():
    with mock.patch(
        "clusterman.aws.spot_prices.get_data_for_price", autospec=True
    ) as mock_get:
        yield mock_get
...
def mock_generate_key():
    with mock.patch(
        "clusterman.aws.spot_prices.generate_key_with_dimensions", autospec=True
    ) as mock_gen:
...
def test_spot_price_generator():
    ...
    assert result_list == [
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 2, 22), "0.1"
        ),
        mock_price_object(
            "c3.xlarge", "fake-az-2a", datetime.datetime(2017, 1, 3, 12, 1, 40), "0.8"
        ),
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 0, 22), "0.1"
        ),
    ]

    # Check EC2 query parameters.
    assert mock_client.get_paginator.call_args_list == [
        mock.call("describe_spot_price_history")
    ]
    assert mock_client.get_paginator.return_value.paginate.call_count == 1
    assert mock_client.get_paginator.return_value.paginate.call_args_list == [
        mock.call(
...
def test_write_prices_with_dedupe_normal(mock_get_data):
    mock_prices = [
        ...
        mock_price_object(
            "c3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 1, 10), "0.4"
        ),
    ]
    ...
    assert mock_writer.send.call_args_list == [
        mock.call(mock_get_data.return_value) for _ in range(3)
    ]
...
def test_write_prices_with_dedupe_multiple(mock_get_data):
    mock_prices = [
        ...
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 5, 39), "0.3"
        ),
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 5, 22), "0.2"
        ),
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 5, 10), "0.1"
        ),
        # Other market in that minute
        mock_price_object(
            "m3.large", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 5, 10), "0.7"
        ),
    ]
    mock_writer = mock.MagicMock()
    spot_prices.write_prices_with_dedupe(iter(mock_prices), mock_writer, 60)
    ...
    assert mock_writer.send.call_args_list == [
        mock.call(mock_get_data.return_value) for _ in range(2)
    ]
...
def test_write_prices_with_dedupe_boundary(mock_get_data):
    mock_prices = [
        ...
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 15, 39), "0.3"
        ),
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 15, 0), "0.2"
        ),
        mock_price_object(
            "m3.xlarge", "fake-az-2b", datetime.datetime(2017, 1, 3, 12, 14, 39), "0.1"
        ),
    ]
    mock_writer = mock.MagicMock()
    spot_prices.write_prices_with_dedupe(iter(mock_prices), mock_writer, 60)
...
def test_get_data_for_price(mock_generate_key):
    ...
    assert value == price
    assert mock_generate_key.call_args_list == [
        mock.call("spot_prices", dimensions=expected_dimensions)
    ]
