...
def batch(extra_args=None, mock_sensu=True):
    with mock.patch("clusterman.batch.autoscaler.setup_config"), mock.patch(
        "clusterman.batch.autoscaler.Autoscaler", signal=mock.Mock()
    ):
...
def test_run(mock_sensu, mock_running, mock_time, mock_sleep, dry_run):
    ...
    with mock.patch("builtins.hash") as mock_hash:
        ...
        batch_obj.run()
    assert batch_obj.autoscaler.run.call_args_list == [
        mock.call(dry_run=dry_run) for i in range(3)
    ]
