...
def mock_setup_config():
    with mock.patch(
        "clusterman.batch.cluster_metrics_collector.setup_config", autospec=True
    ) as mock_setup:
        yield mock_setup
...
def test_configure_initial(
    mock_ls, mock_mesos_pool_manager, mock_client_class, batch, mock_setup_config
):
    pools = ["pool-1", "pool-3", "pool-4"]
    mock_ls.return_value = [f"{p}.mesos" for p in pools[:2]] + [
        f"{p}.kubernetes" for p in pools[2:]
    ]
    with mock.patch(
...
def test_write_metrics(batch):
    batch.pool_managers = {
        "pool_A.mesos": mock.Mock(
            autospec=PoolManager, pool="pool_A", scheduler="mesos"
        ),
        "pool_B.mesos": mock.Mock(
            autospec=PoolManager, pool="pool_B", scheduler="mesos"
        ),
        "pool_B.kubernetes": mock.Mock(
            autospec=PoolManager, pool="pool_B", scheduler="kubernetes"
        ),
    }
    ...
    for pool, manager in batch.pool_managers.items():
        assert manager.cluster_connector.get_resource_allocation.call_args_list == [
            mock.call("cpus")
        ]
...
def test_run(mock_sensu, mock_running, mock_time, mock_sleep, batch):
    ...
    with mock.patch(
        ...
        batch, "write_metrics", autospec=True
    ) as write_metrics, mock.patch(
        ...
        expected_write_metrics_calls = [
            mock.call(writer, metric_to_write.generator, metric_to_write.pools)
            for metric_to_write in METRICS_TO_WRITE
        ] * 4
