...
def mock_setup_config():
    with mock.patch(
        "clusterman.batch.spot_price_collector.setup_config", autospec=True
    ) as mock_setup:
        yield mock_setup
...
def mock_sensu():
    with mock.patch(
        "clusterman.batch.spot_price_collector.sensu_checkin", autospec=True
    ) as sensu:
        yield sensu


def test_start_time_parsing(batch):
    args = batch_arg_parser(
        batch, ["--aws-region", "us-west-1", "--start-time", "2017-09-12T12:11:23"]
    )
    assert args.start_time == datetime.datetime(
        2017, 9, 12, 12, 11, 23, tzinfo=datetime.timezone.utc
    )
...
def test_configure_initial_with_options(batch, mock_client_class, mock_setup_config):
    batch.options = batch_arg_parser(
        batch
    )  # just to set up options object, will override
...
@mock.patch(
    "clusterman.batch.spot_price_collector.write_prices_with_dedupe", autospec=True
)
def test_write_prices(mock_write, mock_price_gen, batch):
    ...
    assert mock_write.call_args_list == [
        mock.call(mock_price_gen.return_value, writer, 60)
    ]
...
def test_run(mock_running, mock_now, mock_time, mock_sleep, batch, mock_sensu):
    ...
    with mock.patch("builtins.hash") as mock_hash, mock.patch.object(
        batch, "write_prices", autospec=True
    ) as write_prices, mock.patch(
        "clusterman.batch.spot_price_collector.logger"
    ) as mock_logger:
        ...
        assert batch.metrics_client.get_writer.call_args_list == [
            mock.call(METADATA) for i in range(4)
        ]
        assert write_prices.call_args_list == [
            mock.call(mock_now.return_value, writer) for i in range(4)
        ]
