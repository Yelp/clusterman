...
def mock_pool_manager():
    return mock.Mock(
        cluster="mesos-test", pool="bar", scheduler="mesos", spec=PoolManager
    )
...
class TestManageMethods:
    ...
    def test_change_target_capacity(
        ...
        assert mock_pool_manager.modify_target_capacity.call_args == mock.call(
            123, dry_run
        )
...
def test_main_error(
    mock_scribe_stream, mock_stale, mock_change, mock_pool_manager, mock_logger, args
):
...
def test_main(
    ...
    if non_action == "mark_stale":
        ...
        assert mock_logger.call_args == mock.call(
            "scribe_stream", f"{LOG_TEMPLATE} changing capacity"
        )
    elif non_action == "target_capacity":
        assert mock_stale.call_count == 1
        assert mock_logger.call_args == mock.call(
            "scribe_stream", f"{LOG_TEMPLATE} marking stale"
        )
