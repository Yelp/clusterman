...
def test_setup_config_cluster(cluster, pool, scheduler, tag, mock_config_files):
    ...
    with mock.patch(
        "clusterman.config.load_cluster_pool_config", autospec=True,
...
def test_setup_config_region_and_cluster():
    args = argparse.Namespace(
        env_config_path="/nail/etc/config.yaml", cluster="foo", aws_region="bar",
    )
    with mock.patch("clusterman.config._load_module_configs"), pytest.raises(
...
def test_setup_config_region(mock_load_module_configs, mock_config_files):
    args = argparse.Namespace(
        env_config_path="/nail/etc/config.yaml", aws_region="fake-region-A",
    )
    config.setup_config(args)
...
@pytest.mark.parametrize(
    "cluster,pool,pool_other_config", [("cluster-B", "pool-1", 200)]
)
def test_load_cluster_pool_config(cluster, pool, pool_other_config, mock_config_files):
    ...
    assert (
        staticconf.read_int("other_config", namespace=pool_namespace)
        == pool_other_config
    )
    assert (
        staticconf.read_string(f"resource_groups", namespace=pool_namespace) == cluster
    )
