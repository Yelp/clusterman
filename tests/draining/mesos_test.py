...
def test_build_maintenance_schedule_payload_no_schedule(
    ...
    actual = build_maintenance_schedule_payload(
        mock.Mock(), hostnames, start, duration, drain=True
    )
...
def test_build_maintenance_schedule_payload_no_schedule_undrain(
    ...
    duration = "3600000000000"
    actual = build_maintenance_schedule_payload(
        mock.Mock(), hostnames, start, duration, drain=False
    )
    assert mock_get_maintenance_schedule.call_count == 1
    assert mock_get_machine_ids.call_count == 1
    assert mock_get_machine_ids.call_args == mock.call(hostnames)
...
@mock.patch(
    "clusterman.draining.mesos.open", create=True, side_effect=IOError, autospec=None
)
...
def test_get_maintenance_schedule():
    ...
    assert mock_operator_api.call_args == mock.call(
        data={"type": "GET_MAINTENANCE_SCHEDULE"}
    )


@mock.patch(
    "clusterman.draining.mesos.build_maintenance_schedule_payload", autospec=True
)
def test_drain(mock_build_maintenance_schedule_payload,):
    ...
    expected_args = mock.call(
        mock_operator_api, ["some-host"], "some-start", "some-duration", drain=True
    )
