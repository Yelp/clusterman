    ...
    with mock.patch("clusterman.draining.queue.json", autospec=True,) as mock_json:
        ...
        assert (
            mock_draining_client.submit_instance_for_draining(
                mock_instance, sender=SpotFleetResourceGroup,
            )
        ...
        mock_draining_client.client.send_message.assert_called_with(
            ...
            MessageAttributes={
                "Sender": {"DataType": "String", "StringValue": "sfr",},
            },
...
def test_submit_host_for_draining(mock_draining_client):
    with mock.patch("clusterman.draining.queue.json", autospec=True,) as mock_json:
        ...
        mock_draining_client.client.send_message.assert_called_with(
            QueueUrl=mock_draining_client.drain_queue_url,
            MessageAttributes={
                "Sender": {"DataType": "String", "StringValue": "aws_2_min_warning",},
            },
            MessageBody=mock_json.dumps.return_value,
        )
...
def test_get_warned_host(mock_draining_client):
    with mock.patch(
        ...
        assert (
            mock_draining_client.get_warned_host()
            is mock_host_from_instance_id.return_value
        )
...
def test_submit_host_for_termination(mock_draining_client):
    with mock.patch("clusterman.draining.queue.json", autospec=True,) as mock_json:
        ...
        mock_draining_client.client.send_message.assert_called_with(
            ...
            DelaySeconds=0,
            MessageAttributes={
                "Sender": {"DataType": "String", "StringValue": "clusterman",},
            },
            MessageBody=mock_json.dumps.return_value,
        )
...
def test_delete_warning_message(mock_draining_client):
    ...
    mock_draining_client.client.delete_message.assert_has_calls(
        [
            mock.call(
                QueueUrl=mock_draining_client.warning_queue_url, ReceiptHandle=1,
            ),
            mock.call(
                QueueUrl=mock_draining_client.warning_queue_url, ReceiptHandle=2,
            ),
        ]
    )
...
def test_delete_terminate_message(mock_draining_client):
    ...
    mock_draining_client.client.delete_message.assert_has_calls(
        [
            mock.call(
                QueueUrl=mock_draining_client.termination_queue_url, ReceiptHandle=1,
            ),
            mock.call(
                QueueUrl=mock_draining_client.termination_queue_url, ReceiptHandle=2,
            ),
        ]
    )
...
def test_process_termination_queue(mock_draining_client):
    with mock.patch(
        ...
        mock_draining_client.draining_host_ttl_cache[
            mock_host.instance_id
        ] = arrow.now()
        ...
        mock_delete_terminate_messages.assert_called_with(
            mock_draining_client, [mock_host]
        )
...
def test_process_drain_queue(mock_draining_client):
    with mock.patch(
        ...
        mock_draining_client.process_drain_queue(mock_mesos_client)
        mock_submit_host_for_termination.assert_called_with(
            mock_draining_client, mock_host, delay=0
        )
...
def test_clean_processing_hosts_cache(mock_draining_client):
    mock_draining_client.draining_host_ttl_cache["i123"] = arrow.get(
        "2018-12-17T16:01:59"
    )
    mock_draining_client.draining_host_ttl_cache["i456"] = arrow.get(
        "2018-12-17T16:02:00"
    )
    with mock.patch(
        "clusterman.draining.queue.arrow", autospec=False
    ) as mock_arrow, mock.patch("clusterman.draining.queue.DRAIN_CACHE_SECONDS", 60):
...
def test_process_warning_queue(mock_draining_client):
    with mock.patch(
        ...
        mock_delete_warning_messages.assert_called_with(
            mock_draining_client, [mock_host]
        )
        ...
        mock_draining_client.get_warned_host = mock.Mock(return_value=mock_host)
        mock_draining_client.process_warning_queue()
        mock_submit_host_for_draining.assert_called_with(
            mock_draining_client, mock_host
        )
        mock_delete_warning_messages.assert_called_with(
            mock_draining_client, [mock_host]
    ...
    with mock.patch(
        ...
        assert (
            host_from_instance_id(
                sender="aws", receipt_handle="rcpt", instance_id="i-123",
            )
            is None
        )
