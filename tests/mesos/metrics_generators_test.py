...
def mock_pool_manager():
    ...
    mock_pool_manager.cluster_connector = mock.Mock(
        cluster="mesos-test", pool="bar", scheduler="mesos"
    )
...
def test_generate_system_metrics(mock_pool_manager):
    ...
    mock_pool_manager.cluster_connector.get_resource_allocation.side_effect = (
        resources_allocated.get
    )
    ...
    assert sorted(generate_system_metrics(mock_pool_manager)) == sorted(
        expected_metrics
    )
...
def test_generate_simple_metadata(mock_pool_manager):
    ...
    mock_pool_manager.cluster_connector.get_resource_total.side_effect = (
        resource_totals.get
    )
    ...
    assert sorted(generate_simple_metadata(mock_pool_manager)) == sorted(
        expected_metrics
    )
