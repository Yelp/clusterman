...
def test_no_signal_configured():
    with staticconf.testing.MockConfiguration({}, namespace="bar.mesos_config",), mock.patch(
...
def test_evaluate_signal_connection_errors(mock_signal, conn_response):
    ...
    with mock.patch("clusterman.signals.external_signal.get_metrics_for_signal", return_value={}), pytest.raises(
...
def test_evaluate_restart_dead_signal(mock_signal):
    ...
    with mock.patch(
        "clusterman.signals.external_signal.ExternalSignal._connect_to_signal_process"
    ) as mock_connect, mock.patch("clusterman.signals.external_signal.get_metrics_for_signal", return_value={}):
