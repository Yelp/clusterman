...
def mock_ssfrg(ssfrg_config):
    ...
    instances = [
        Instance(TEST_MARKET, arrow.get(0), join_time=arrow.get(0)) for i in range(10)
    ]
...
def test_get_agent_metadata(mock_cluster_connector):
    instance = list(
        mock_cluster_connector.simulator.aws_clusters[0].instances.values()
    )[0]
    ...
    assert mock_cluster_connector.get_agent_metadata(
        instance.ip_address
    ) == AgentMetadata(
...
def test_simulated_agents(mock_cluster_connector):
    assert (
        mock_cluster_connector.get_resource_total("cpus")
        == 10 * get_market_resources(TEST_MARKET).cpus
    )
    assert (
        mock_cluster_connector.get_resource_total("mem")
        == 10 * get_market_resources(TEST_MARKET).mem
    )
    assert (
        mock_cluster_connector.get_resource_total("disk")
