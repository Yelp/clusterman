...
def test_compute_market_residuals_new_fleet(spot_fleet, test_instances_by_market):
    target_capacity = 10
    residuals = spot_fleet._compute_market_residuals(
        target_capacity, test_instances_by_market.keys()
    )
    assert residuals == list(
        zip(
            sorted(
                list(test_instances_by_market.keys()),
                key=lambda x: spot_fleet.simulator.instance_prices[x].call(
                    spot_fleet.simulator.current_time
                ),
            ),
            [target_capacity / len(test_instances_by_market)]
            * len(test_instances_by_market),
        )
    )
...
def test_market_capacities(
    spot_fleet_request_config, spot_fleet, test_instances_by_market
):
    spot_fleet.modify_size(test_instances_by_market)
    for i, (market, instance_count) in enumerate(test_instances_by_market.items()):
        assert (
            spot_fleet.market_capacities[market]
            == instance_count
            * spot_fleet_request_config["LaunchSpecifications"][i]["WeightedCapacity"]
        )
...
def test_terminate_instance(spot_fleet, test_instances_by_market):
    ...
    terminate_instances_ids = [
        instance.id for instance in added_instances[split_point:]
    ]
...
def test_modify_target_capacity(dry_run, spot_fleet):
    ...
    spot_fleet.modify_target_capacity(
        spot_fleet.target_capacity - capacity, dry_run=dry_run
    )
