...
@pytest.mark.parametrize(
    "early_bp,late_bp", [(False, False), (False, True), (True, False), (True, True),]
)
def test_compute_instance_cost_multi_breakpoints(
    simulator, mock_instance, fn, early_bp, late_bp
):
    ...
    simulator._compute_instance_cost(mock_instance)
    assert (
        simulator.total_cost == fn.call(arrow.get(0)) / 2 + fn.call(arrow.get(1800)) / 2
    )
...
def test_compute_instance_cost_long_breakpoint_gap(simulator, mock_instance, fn):
    ...
    assert simulator.total_cost == (
        (1 / 6 + 3 * 5 / 12) if mock_instance.end_time < arrow.get(3000) else 3
    )
...
def test_compute_instance_cost_outbid(simulator, mock_instance, fn, refund):
    ...
    simulator._compute_instance_cost(mock_instance)
    assert (
        simulator.total_cost == 1 / 2
        if (refund and mock_instance.end_time < simulator.end_time)
        else 1
    )


@pytest.mark.parametrize("refund", [True, False])
def test_compute_instance_cost_outbid_refund_irrelevant(
    simulator, mock_instance, fn, refund
):
    fn.add_breakpoint(mock_instance.end_time.shift(minutes=1), 3)
    mock_instance.bid_price = 2
    simulator.instance_prices[mock_instance.market] = fn
