...
def mock_clog():
    sys.modules[
        "clog"
    ] = mock.Mock()  # clog is imported in the main function so this is how we mock it
    yield
...
def test_ask_for_choice():
    with mock.patch(
        "builtins.input", side_effect=["asdf", "-1", "20", "2"]
    ) as mock_input:
...
def test_parse_time_string_non_arrow():
    ...
    actual_timestamp = (
        arrow.now().replace(tzinfo="US/Eastern").shift(hours=-1).timestamp
    )
...
class TestSensu:
    ...
    @pytest.mark.parametrize(
        "app,pool,scheduler", [(None, None, None), ("bar", "foo", "mesos")]
    )
